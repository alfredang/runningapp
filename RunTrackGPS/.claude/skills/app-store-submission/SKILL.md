...
note in the repo (identity + the marketing copy). **Values below are filled in for RunTrack GPS.**

```
App name:        RunTrack GPS
App ID (ASC):                             # numeric; resolved from bundle id if blank
Bundle ID:       com.tertiaryinfotech.runtrackgps
iCloud container: (none — no CloudKit; runs stored locally in UserDefaults)
Team ID:         GU9WTSTX9M               # Tertiary Infotech (paid) — same team as NotePad
...
> Project-specific notes for RunTrack GPS:
...
> - Permissions to mention in App Review notes: Location (Always, for background run tracking),
>   Microphone + Speech (voice commands). App Privacy → declare **Location (App Functionality,
>   not linked to identity, not used for tracking)**; everything else is **Data Not Collected**.
> - Pre-flight already done in-repo: 1024 icon (no alpha), `ITSAppUsesNonExemptEncryption=false`,
>   `UIRequiredDeviceCapabilities=arm64`, `PrivacyInfo.xcprivacy`, all usage strings, `ExportOptions.plist`.
...
```
Subtitle:    GPS run tracker with voice
Keywords:    running,gps,run tracker,jogging,pace,distance,fitness,route,workout,marathon
Promo text:  Track every run by GPS with a live route map, distance & pace goals, voice
             commands, and spoken feedback — even with the screen locked.
Description: RunTrack GPS is a clean, lightweight running companion. Pick a 5/10/20/40 km
             goal or set your own, then start running. Watch your route draw live on the
             map while distance, pace, and time update in real time. Control your run
             hands-free with voice commands (start, pause, resume, stop) and hear spoken
             milestones at every kilometre, the halfway point, 90%, and your goal. GPS,
             timer, and audio feedback keep working in the background, so you can lock
             your phone and just run.
```
